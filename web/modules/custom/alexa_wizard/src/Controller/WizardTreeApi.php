...
class WizardTreeApi extends ControllerBase {
  ...
  public function updateWizardtree(Request $request) : Response {
    ...
    $status = \Drupal::service('alexa_wizard.wizard')->saveWizardTree( $postData );
    // TODO parse status
    ...
    if ( $rootStepId !== null ) {
      $response->setContent(json_encode(\Drupal::service('alexa_wizard.wizard')->buildFlattenedWizardTreeFromNodeId($rootStepId)));
    } else {
      $response->setContent(json_encode(\Drupal::service('alexa_wizard.wizard')->buildFlattenedWizardTree()));
    }
    return $response;
  }
  ...
  public function updateWizardTreeAccess(AccountInterface $account) : AccessResultInterface {
      ...
      return AccessResult::allowedIf(\Drupal::service('alexa_wizard.wizard')->validateUserWizardTreePermissions($account));
  }
  ...
  /**
    ...
    public function getFlattenedWizardTree(Request $request, int|null $rootId) : JsonResponse {
      ...
      if ( $rootId !== null ) {
        $tree = \Drupal::service('alexa_wizard.wizard')->buildFlattenedWizardTreeFromNodeId( $rootId );
      } else {
        $tree = \Drupal::service('alexa_wizard.wizard')->buildFlattenedWizardTree();
      }
      // TODO parse status
