...
/**
 * Controller to handle route theming for the Wizards module.
 */
class WizardController extends ControllerBase {
  ...
  public function wizardPage(string $wizardId) : array {
    ...
    $wizardTree = \Drupal::service('wizards.wizard')->buildFlattenedWizardTreeFromNodeId( $wizardId );
    // Determine the Wizard update path.
    // TODO make this absolute instead of relative.
    $wizardUpdatePath = \Drupal\Core\Url::fromRoute('wizards.wizard_tree.update.v1')->toString();

    // Set the theme to allow for a custom template to be loaded, set JS variables,
    // and attach the necessary library.
    return [
      '#theme' => 'wizards_wizard',
      '#attached' => [
        'library' => [
          'wizards/wizards.react_wizard_viewer',
        ],
  ...
  public function wizardSelectPage() : array {
    // Load all wizards
    $availableWizards = \Drupal::service('wizards.wizard')->getAllWizards();
    $reactWizards = [];
    ...
    return [
      '#theme' => 'wizards_wizard_select',
      '#attached' => [
        'library' => [
          'wizards/wizards.react_wizard_select',
        ],
        // JS variables go here
        'drupalSettings' => [
